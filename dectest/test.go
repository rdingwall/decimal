...
func execute(t *testing.T, c *Case) {
	...
	if c.Clamp {
		t.Skip("unsupported directive: clamp")
	}
	...
	if !ok {
		t.Skipf("unsupported condition(s): %s", c.Conditions)
	}

	mode, ok := decRoundingModes[c.Mode]
	if !ok {
		t.Skipf("unsupported rounding mode: %s", c.Mode)
	}
	...
	if nfn, ok := nilary[c.Op]; ok {
		...
		default:
			t.Skipf("unknown op: " + c.Op.String())
		}
	}
}
